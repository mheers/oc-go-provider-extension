...
- **Show API Key action.** The management menu (status bar / "OpenCode Go: Manage OpenCode Go Provider") now has a "Show API Key…" item that reveals the currently stored OpenCode Go API key in a modal with a "Copy to Clipboard" button (with a read-only document fallback for manual selection). Also available directly from the Command Palette as **OpenCode Go: Show API Key**. The key is read from VS Code's `SecretStorage`, which is encrypted at rest by the platform (libsecret / Keychain / DPAPI).
